...
class GitLabApiService {
  ...
  async getGroupProjects(groupId) {
    try {
      const response = await fetch(`${this.baseUrl}/groups/${groupId}/projects?include_subgroups=true&per_page=100`, {
        headers: {
  ...
  async getTestReports(projectId) {
    try {
      const response = await fetch(`${this.baseUrl}/projects/${projectId}/pipelines/latest/test_report`, {
        headers: {
          'PRIVATE-TOKEN': this.privateToken
        }
      });
      ...
      console.error(`Failed to fetch test reports for project ${projectId}:`, error);
